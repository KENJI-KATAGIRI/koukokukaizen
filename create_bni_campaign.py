...
def meta_post(path, **data):
    ...
    try:
        ...
        raise RuntimeError(f"API Error (HTTP {e.code}) on {path}:\n{err_body}")
    if 'error' in res:
...
TARGETING = {
    ...
    'targeting_automation': {'advantage_audience': 0},  # オーディエンス拡張オフ（191件に厳密配信）
...
if not DRY_RUN:
    camp = meta_post(
        ...
        is_adset_budget_sharing_enabled='false',  # CBO不使用・予算はアドセット側（API v25で必須）
    )
    camp_id = camp['id']
...
if not DRY_RUN:
    adset = meta_post(
        ...
        bid_strategy='LOWEST_COST_WITHOUT_CAP',  # 自動入札（上限なし）・入札価格指定不要
